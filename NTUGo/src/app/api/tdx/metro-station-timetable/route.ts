...
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = filterQuery 
      ? `https://tdx.transportdata.tw/api/basic/v2/Rail/Metro/StationTimeTable/TRTC?${filterQuery}&$format=JSON`
      : `https://tdx.transportdata.tw/api/basic/v2/Rail/Metro/StationTimeTable/TRTC?$format=JSON`;
    ...
    // 調試：記錄返回的數據結構
    console.log('StationTimeTable API 返回數據:', {
      isArray: Array.isArray(data),
      length: timetable.length,
      firstItem: timetable.length > 0 ? timetable[0] : null,
      firstItemKeys: timetable.length > 0 ? Object.keys(timetable[0]) : [],
      firstItemFull: timetable.length > 0 ? JSON.stringify(timetable[0], null, 2) : null,
    });
    ...
    return NextResponse.json(
      { error: '獲取捷運列車時刻表失敗', message: error.message },
      { status: 500 }
    );
  }
}
